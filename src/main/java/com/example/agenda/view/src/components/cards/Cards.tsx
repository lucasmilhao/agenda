...
export function Cards(user : ContatoProps) {
    ...
    return (
        <div className="card-container">
            <div className="information">
                ...
                <h2>{user.telefone}</h2>
                <p>{user.email}</p>
                <p>{new Date(user.dataNascimento).toLocaleDateString()}</p>
            </div>
            <div className="config">
                <button onClick={() => Swal.fire({
                    ...
                    if(result.isConfirmed) deletarContato(user.id);
                })
                }>
            ...
            {isEditModalOpen && <ModalEdit user={user} closeModal={handleOpenEditModal}></ModalEdit>}  
        </div>
    )
}